...
@Component({
  ...
  `<div class="card">
    ...
    <div class="card-footer" [hidden]="isCompleted">
        <button type="button" class="btn btn-secondary float-left" (click)="previous()" [hidden]="!hasPrevStep || !activeStep.showPrev">{{activeStep.previousValue}}</button>
        <button type="button" class="btn btn-secondary float-right" (click)="next()" [disabled]="!activeStep.isValid" [hidden]="!hasNextStep || !activeStep.showNext">{{activeStep.nextValue}}</button>
        <button type="button" class="btn btn-secondary float-right" (click)="complete()" [disabled]="!activeStep.isValid" [hidden]="hasNextStep">{{activeStep.doneValue}}</button>
    </div>
  </div>`
  ,
  styles: [
...
export class WizardComponent implements AfterContentInit {
  ...
  private _previousValue: string = 'Previous';
  private _nextValue: string = 'Next';
  private _doneValue: string = 'Done';
  ...
  get doneValue(): string {
    return this._doneValue;
  }
  ...
  public previous(): void {
    if (this.hasPrevStep) {
      ...
      this.activeStep.onPrev.emit(prevStep);
